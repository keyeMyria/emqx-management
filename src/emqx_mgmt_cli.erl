...
-export([status/1, broker/1, cluster/1, clients/1, sessions/1,
         routes/1, subscriptions/1, plugins/1, bridges/1, edge/1,
...
edge(["bridge", "status"]) ->
    foreach(fun({Name, State}) ->
                    emqx_cli:print("name: ~s     status: ~s~n", [Name, State])
                    end, emqx_edge_bridge_sup:bridges());
edge(["static", "start"]) ->
    emqx_edge:start(static);
    
edge(["static", "stop"]) ->
    emqx_edge:stop(static),
    ?PRINT_MSG("stop edge successfully.");
edge(["list"]) ->
    ?PRINT_MSG("list edge successfully");
edge(["add", Host]) ->
    ?PRINT_MSG("add edge successfully");
edge(["remove"]) ->
    ?PRINT_MSG("add edge succecc");
edge(_) ->
    emqx_cli:usage([{"edge start",                 "start edge cluster"},
                    {"edge list",                  "list edge clusters"},
                    {"edge list <Name>",           "list nodes of specified edge cluster"},
                    {"edge add <Name> <Hosts>",    "add hosts to specified edge cluster"},
                    {"edge remove <Name> <Hosts>", "remove hosts from specified edge cluster"}
                   ]).

%% %%--------------------------------------------------------------------
%% %% @doc Edge Queue commands
%% edge_queue(["create", Name, Topics]) ->
%%     ?PRINT_MSG("create edge queue successfully");
%% edge_queue(["list"]) ->
%%     ?PRINT_MSG("list edge queue successfully");
%% edge_queue(["remove"]) ->
%%     ?PRINT_MSG("remove edge queue successfully");
%% edge_queue(_) ->
%%     emqx_cli:usage([{"edge_queue create <Topic>", "create edge queues"},
%%                     {"edge_queue list"                 , "list edge queues"},
%%                     {"edge_queue remove <Topic>"        , "remove specified edge queue"}
%%                    ]).
